...
class BedtoolsGenomeCov(object):
    ...
    Usage:
        bed.gencov -i <input> -o <output> [-s <strand>] [--bam] [-b] [--split] [-n <name>] [-t <threads>]
...
class BedtoolsBedgraphBigwig(object):
    ...
    Usage:
        bed.bgtobw -i <input> -o <output> -g <genome_fai> [-n <name>] [-t <threads>]
...
class BigwigPipeline(object):
    ...
    usage:
         grape.bigwig -i <bam_file> -o <bw_file> -g <genome_file> [-t <threads>] [--stranded] [ --reverse-first-mate | --reverse-second-mate ]
    ...
    def pipeline(self):
        ...
        if self.options['reverse_first_mate']:
            bam = p.run('grape_samtools_view', input=bam, threads=self.threads, name=sample) | \
